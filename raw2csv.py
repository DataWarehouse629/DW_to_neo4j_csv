...
with open(raw_file_path, 'r', encoding='ISO-8859-1') as raw_file:
    with open(user_file_path, 'w', encoding='utf-8', newline='') as user_file, \
        ...
        user_file.write('userId:ID(User-ID),profileName:string\n')
        reviews_file.write(':START_ID(User-ID),:END_ID(Product-ID)\n')
        ...
        for line in tqdm.tqdm(raw_file):
            if (line.startswith('p')):
                product_id = line[k_product_start:-1]
                line = next(raw_file)
                user_id = line[k_user_start:-1]
                line = next(raw_file)
                try:
                    profile_name: str = line[k_name_start: -1].replace('\n', '')
